...
#[inline]
pub fn surface_as_toplevel(surface: gdk::Surface) -> Option<gdk::Toplevel> {
  surface.clone().downcast::<gdk::Toplevel>().ok()
}
  ...
  default_pointer(&window.display())
    .and_then(|pointer| {
      window
        ...
        .and_then(|(x, y, _)| {
          Some(LogicalPosition::new(x, y).to_physical::<f64>(window.scale_factor() as _))
        })
    })
...
pub fn set_size_constraints<W: GtkWindowExt + WidgetExt>(
  ...
  if min_size.width > 0 && min_size.height > 0 {
    window.set_size_request(min_size.width, min_size.height);
  }
}
