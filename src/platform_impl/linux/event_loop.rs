      ...
      while let Ok((id, request)) = window_requests_rx.recv().await {
        if let Some(window) = app_.window_by_id(id.0) {
          match request {
            ...
            WindowRequest::Focus => {
              window.present_with_time(gdk::CURRENT_TIME as _);
            }
            ...
            WindowRequest::DragWindow => {
              ...
              if let (Some(cursor), Some(surface)) = (cursor, surface) {
                ...
                if let (Some(toplevel), Some((x, y, _))) = (toplevel, pos) {
                  toplevel.begin_move(
                    &cursor,
            ...
            WindowRequest::WindowIcon(_icon) => {
              todo!()
              //if let Some(icon) = window_icon {
              //  window.set_icon_name(Some(&icon.inner.into()));
              //}
            }
            ...
            WindowRequest::WireUpEvents {
                  ...
                  && !window_clone.is_maximized()
                {
                  ...
                  match edge {
                    ...
                    _ => {
                      window_clone
                        .surface()
                        .and_then(|s| util::surface_as_toplevel(s))
                        .map(|toplevel| {
              ...
              let keyboard_handler = Rc::new(move |key: gdk::Key, keycode, element_state| {
                ...
                let mut mods = keyboard::get_modifiers(key.clone(), keycode);
                if !mods.is_empty() {
